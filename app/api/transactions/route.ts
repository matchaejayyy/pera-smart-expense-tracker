...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { merchant?: string; amount?: number; type?: "INCOME" | "EXPENSE" | "SAVINGS"; accountId?: string; categoryId?: string | null; savingsGoalId?: string | null; bookedAt?: string; countsTowardBudget?: boolean };
  const amount = Number(body.amount);
  if (!Number.isFinite(amount) || amount <= 0 || !body.type || !body.accountId) return NextResponse.json({ error: "Account, amount, and transaction type are required." }, { status: 400 });
  ...
  const account = await context.prisma.account.findFirst({ where: { id: body.accountId, ownerId: context.user.id, isArchived: false } });
  if (!account) return NextResponse.json({ error: "Account not found. Create or choose an active account first." }, { status: 404 });
  ...
  const transaction = await context.prisma.$transaction(async (prisma) => {
    const created = await prisma.transaction.create({
      data: { ownerId: context.user.id, accountId: account.id, categoryId: category?.id ?? null, savingsGoalId: savingsGoal?.id ?? null, merchant, amount, type: body.type!, bookedAt: body.bookedAt ? new Date(body.bookedAt) : new Date(), countsTowardBudget: body.type === "EXPENSE" ? body.countsTowardBudget !== false : false },
      include: { category: true, savingsGoal: true, account: true },
    });
    if (savingsGoal) await prisma.savingsGoal.update({ where: { id: savingsGoal.id }, data: { currentAmount: { increment: amount } } });
    return created;
  });
...
export async function PATCH(request: Request) {
  ...
  const body = await request.json() as { id?: string; merchant?: string; amount?: number; type?: "INCOME" | "EXPENSE" | "SAVINGS"; accountId?: string; categoryId?: string | null; savingsGoalId?: string | null; bookedAt?: string; countsTowardBudget?: boolean };
  const amount = Number(body.amount);
  if (!body.id || !body.accountId || !Number.isFinite(amount) || amount <= 0 || !body.type) return NextResponse.json({ error: "Transaction, account, amount, and type are required." }, { status: 400 });
  if (body.type === "EXPENSE" && (!body.merchant?.trim() || !body.categoryId)) return NextResponse.json({ error: "Merchant and category are required for expenses." }, { status: 400 });
  if (body.type === "SAVINGS" && !body.savingsGoalId) return NextResponse.json({ error: "Choose a savings destination." }, { status: 400 });
  ...
  const account = await context.prisma.account.findFirst({ where: { id: body.accountId, ownerId: context.user.id, isArchived: false } });
  if (!account) return NextResponse.json({ error: "Account not found. Choose an active account." }, { status: 404 });
  ...
  const data = await context.prisma.$transaction(async (prisma) => {
    ...
    const updated = await prisma.transaction.update({
      ...
      data: { merchant, accountId: account.id, amount, type: body.type!, categoryId: category?.id ?? null, savingsGoalId: savingsGoal?.id ?? null, countsTowardBudget: body.type === "EXPENSE" ? body.countsTowardBudget !== false : false, ...(body.bookedAt ? { bookedAt: new Date(body.bookedAt) } : {}) },
      include: { category: true, savingsGoal: true, account: true },
    });
    if (savingsGoal) await prisma.savingsGoal.update({ where: { id: savingsGoal.id }, data: { currentAmount: { increment: amount } } });
    return updated;
