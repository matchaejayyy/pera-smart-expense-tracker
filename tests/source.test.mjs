...
test("keeps the Pera product, CRUD flows, account controls, RLS, and standard Next.js scripts", async () => {
  const [dashboard, landing, login, callback, confirmation, protectedPage, profilePage, profileClient, transactionApi, recurringApi, budgetApi, categoryApi, savingsApi, smartTipsApi, accountApi, authProxy, prismaSchema, rlsMigration, savingsMigration, optionalSavingsTargetMigration, migrationHistorySecurity, packageJson] = await Promise.all([
  ...
  assert.match(dashboard, /const monthlyBudget = totals\.income/);
  assert.match(dashboard, /Automatic from income/);
  ...
  assert.match(dashboard, /Available \{exactCurrency\(Math\.max\(0, monthlyBudget - totalSpent\)\)\}/);
